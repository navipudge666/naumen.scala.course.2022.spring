...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        for { i <- 2 to number
            if (2 to sqrt(i).toInt).forall(j => i % j != 0)  && number % i == 0
        } yield i;
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.map(el => (el._1, getBallWeight(el._2._1, el._2._1))).sortBy(_._2).map(el => el._1);
    }

    def getBallWeight(radius: Int, density: Double) = 4/3*Math.PI*Math.pow(radius, 3)*density;
}